  ...
  async function handleSubscribe() {
    ...
    try {
      ...
      const options = {
        ...
        modal: { ondismiss: () => {} },
        handler: () => {
          // Webhook activates subscription; redirect to dashboard
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col px-5 pt-12 pb-10 relative">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-[12px] mt-0.5 ${selected === p.id ? 'text-white/70' : 'text-[#8E8E93]'}`}>
                  {p.perMonth}
                  {p.badge && (
                    <span className={`ml-2 font-semibold px-1.5 py-0.5 rounded-md text-[11px] ${selected === p.id ? 'bg-white/20 text-white' : 'bg-[#6B0F1A]/10 text-[#6B0F1A]'}`}>
                      {p.badge}
                    </span>
                  )}
                </p>
      ...
      <button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-2 bg-[#6B0F1A] text-white rounded-2xl py-4 text-[17px] font-bold shadow-sm active:scale-[0.98] transition-transform disabled:opacity-70"
      >
        {loading ? (
